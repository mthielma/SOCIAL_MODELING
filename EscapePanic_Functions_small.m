...
agent_sensitivity   = 0; % reduce velocity by which factor if agent is on node? [1 - do not take agents into account, 1e-8 - agents in the way have ahuge influence]
...
%     % ExitForceY = Parameter.ExitFactor*Parameter.A.*DistExitY./DistExitTot;
    ...
    % move agents
    %----------------------------------------------------
    [AGENT] = MoveAgents(AGENT,X_Grid,Y_Grid,Gradient_x,Gradient_y,dt,nagent);
