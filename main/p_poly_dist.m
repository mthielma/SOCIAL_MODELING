...
% Revision history:
%    25/11/2011 - vectorized the function to use with multiple input points
...
% xp = x - (A.*AB).*vv;
% yp = y - (B.*AB).*vv;
vv = (bsxfun(@times,A,x)+bsxfun(@times,B,y)+C(:,ones(1,no_points)));
AAB = (A.*AB);
xp  = x(ones(1,4),:) - AAB(:,ones(1,no_points)).*vv;
yp  = y(ones(1,4),:) - AAB(:,ones(1,no_points)).*vv;

% Test for the case where a polygon rib is 
% either horizontal or vertical. From Eric Schmitz
id = find(diff(xv)==0);
xp(id,:)=xv(id,ones(1,no_points));
clear id
id = find(diff(yv)==0);
yp(id,:)=yv(id,ones(1,no_points));
